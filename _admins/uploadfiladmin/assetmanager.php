...
$AllowedTypes = "gif|jpg|png|swf|doc|flv|pdf";
//$AllowedTypes = "*";
...
Function writeFolderSelections()
	...
	echo "<select name='selCurrFolder' id='selCurrFolder' onchange='changeFolder()' class='inpSel' style='width:250px'>";
...
function writeFileSelections()
	...
	echo "<div style='overflow:auto;width:100%;height:450px;margin-top:3px;margin-bottom:0px;'>";
		...
		if($sItem=="."||$sItem=="..")
			...
			if(!$fIsDirectory) 
				...
				if($ffilter=="flash")
					...
					{
					if ($sExt=="swf" || $sExt=="flv" || $sExt=="avi" || $sExt=="wmv" || $sExt=="mpg" || $sExt=="mpeg" || $sExt=="wav" || $sExt=="wma" || $sExt=="mid" || $sExt=="mp3") $bDisplay=true;
					}
				...
				if($bDisplay)
					...
					if($sExt=="gif" ||$sExt=="jpg"||$sExt=="jpeg"||$sExt=="bmp"||$sExt=="png"||$sExt=="ico")
					{
					    echo "<td onclick=\"selectFile(".$nIndex.")\"  valign=bottom width='100%' ><u id=\"idFile".$nIndex."\"><img src='image.php/image.jpg?image=/".$currFolder."/".$sItem."&height=30&cropratio=2:1'><span style='padding-left:5px;'>".$sItem."</span></u>&nbsp;&nbsp;<img style='cursor:pointer;' onclick=\"downloadFile(".$nIndex.")\" src='download.gif'></td>";
					}else
					{
						echo "<td onclick=\"selectFile(".$nIndex.")\"  valign=bottom width='100%' ><u id=\"idFile".$nIndex."\"><span style='padding-left:5px;'>".$sItem."</span></u>&nbsp;&nbsp;<img style='cursor:pointer;' onclick=\"downloadFile(".$nIndex.")\" src='download.gif'></td>";
					}
					...
					if(!$bWriteFolderAdmin)
						{
                            ...
                            echo "<img alt='Delete' width='12' src='unchecked.gif' border='0' />";
						}
					echo "</u></td>";
	...
	if($bFileFound==false)
		echo "<tr><td colspan=4 align='center'><script>document.write(getTxt('Empty...'))</script></td></tr></table></div>";
	else
...
*/
<?php
if(isset($_GET['type'])&&($_GET['type']=="1"))// neu la hinh anh thi object su dung thuoc tinh src
{
?>
	function doOk()
	{
	<?php
		if(isset($_GET['object1'])&&(isset($_GET['object2'])))//object1 là hidden ; object2 là picture
		{		
	?>
			var object1="<?=$_GET['object1']?>";
			object2="<?=$_GET['object2']?>";
			var ext=inpSource.value.substr(parseInt(inpSource.value.length)-3,3);
				<?php
					if(subdomain=="0")
					{
					?>
						var mangItems=inpSource.value.substr(1).split("/");
						var firstItems=mangItems.shift();//loai bo phan tu dau mang a[0]
						var valuePath="/"+mangItems.join("/");	// noi lai mang
				<?php
					}else
					{
				?>
						var valuePath=inpSource.value;
				<?php
					}
				?>
			if(ext=="swf")
			...
			{
			    window.opener.document.getElementById(object2).src="resize.php?from=<?=pathPicture?>"+valuePath+"&w=50";
			}
			window.opener.document.getElementById(object1).value=valuePath;
			bOk=true;
			close();
...
function doOk()
	...
	?>
			...
			if(navigator.appName.indexOf('Microsoft')!=-1)
			{
				object1=eval("dialogArguments.frmp."+object1);
				object1.value=valuePath;
				bOk=true;
				self.close();
			}else{
				window.opener.document.getElementById(object1).value=valuePath;
...
<td valign=top style="background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
		...
		<td valign=top align="center">
				...
				<tr>
				<td>
					<div id="idPreview" style="margin-right:10px;overflow:auto;width:500px;height:440px;border:#999 1px solid;background:#ffffff"></div>
					<div><input type="text" id="inpSource" name="inpSource" style="margin-top:3px;border:#999 1px solid;width:402px" class="inpTxt"></div>
				</td>
...
<tr>
<td class="dialogFooter" style="height:25px;padding-right:15px;" align=right valign=middle>
